...
florida$size_ha <- florida$sizw * 0.09
...
for(i in seq_along(disturbences_names)){
  subset <- single_disturbence
  percent_event <- dim(subset)[1] / dim(final)[1]
  percent_area <- sum(subset$size_ha)/ sum(final$size_ha)
...
# 
# seq <- c(1:length(final[final$state == "FL",]$sizw))
...
# final %>% 
#   filter(state == "FL") %>%
# ggplot( aes(size_ha, ratio), show.legend = TRUE) +
#   geom_hex() + 
#   geom_line(aes(x =seq * 0.09 , y = (upper )), color = "black")+
#   geom_line(aes(x =seq * 0.09 , y = (lower )), color = "black")+
#    geom_density_2d(color = "dark green")+ 
#   scale_x_continuous(trans="log") +
#   scale_fill_gradient(trans = "log") + 
#   xlab("Area ha") +
...
# 
# 
# seq <- c(1:length(final[final$state == "OR",]$sizw))
# seq[length(seq)] <- max(oregon$sizw)
# lower <- lower_bound_ratios(seq)
# upper <- upper_bound_ratios(seq)
# 
...
# ggplot(unknown_only, aes(x = size_ha)) +
#   geom_density( fill = "light blue") +
#   xlab("Area ha") +
# scale_x_continuous(trans="log")
#  
# ggplot(unknown_only, aes(x = size_ha)) +
#   geom_density(data = other_mechanical, aes(x = size_ha), fill = "green", alpha = 0.5, position = "stack", show.legend = TRUE)+
#   geom_density(data = thining_only, aes(x = size_ha), fill = "grey", alpha = 0.5, position = "stack")+
#   geom_density(data = unknown_only, fill = "light blue", alpha = 0.5, position = "stack") +
#   geom_density(data = Prescribed ,aes(x = size_ha), fill = "brown", alpha = 0.5, position = "stack")+
#   geom_density(data = wildfire ,aes(x = size_ha), fill = "orange", alpha = 0.5, position = "stack")+
#   geom_density(data = wildland, aes(x = size_ha), fill = "yellow", alpha = 0.5, position = "stack")+
#   scale_fill_manual(name  ="Disturbence type", labels=c("Thinning", "Unknown", "Prescribed", "Wildfire", "Wildland"))+
# scale_x_continuous(trans="log") +
#   theme(legend.position="right")+ 
# xlab("Area ha") 

# ggplot(single_disturbence,aes(x = size_ha)) +
#   geom_density(data = single_disturbence[conditional_fl_8_3,], aes(x = size_ha, group = dist_name), fill = "green", alpha = 0.25, position = "stack", show.legend = TRUE)+
#   geom_density(data = single_disturbence[conditional_fl_8_5,], aes(x = size_ha), fill = "light blue", alpha = 0.25, position = "stack")+
#   geom_density(data =single_disturbence[conditional_or_6_2,], aes(x = size_ha), fill = "orange", alpha = 0.25, position = "stack")+
#   geom_density(data =single_disturbence[conditional_or_7_1,], aes(x = size_ha), fill = "yellow", alpha = 0.25, position = "stack")+
# scale_x_continuous(trans="log") +
#   theme(legend.position="right")+ 
# xlab("Area ha") 
...
# ggplot(single_disturbence,aes(x = size_ha)) +
#   geom_ri( aes(x = size_ha, group = eco_II, fill = eco_II),position = "stack" , show.legend = TRUE)+
# scale_x_continuous(trans="log") +
#   theme(legend.position="right")+ 
#   #scale_fill_manual()+ 
# xlab("Area ha") 
...
# ### By Disturbence type
# single_disturbence %>%
#   filter(single_disturbence$dist_name == disturbences_names) %>%
# ggplot(aes(x = size_ha)) +
#   geom_density( aes(x = size_ha, group = dist_name, fill= dist_name,), position = "stack", show.legend = TRUE)+
# scale_x_continuous(trans="log") +
#   theme(legend.position="right")+ 
#   #scale_fill_manual()+ 
# xlab("Area ha") 
...
# scale_colour_discrete(name  ="Disturbence type",
#                             breaks=unique(single_disturbence$dist_name),
#                             labels=unique(single_disturbence$dist_name))+ 
#   
...
# na.omit(single_disturbence[single_disturbence$dist_name %in% disturbences_names, ]) %>%
# ggplot( aes(x = size_ha, y = dist_name)) + scale_x_continuous(trans="log")+geom_density_ridges() + geom_vline(xintercept = 0.09) + geom_vline(xintercept =  7)
...
# unknown_only <- unknown_only[unknown_only$sizw != 1 & unknown_only$sizw !=2 & unknown_only$sizw != 3,]
# mastication <- mastication[mastication$sizw >3, ]
# herbicide <- herbicide[herbicide$sizw >3,]
#no_unknowns_single_disturbence[no_unknowns_single_disturbence$sizw]

#dists <- list(clearcut_only, wildland, wildfire, prescibed, thining_only, other_mechanical, unknown_only)
```
...
#KS_results$pval[KS_results$pval <  0.0001 ] <- " < 0.0001 "

reciprical_name <- function(x){
  first_name <- grep(" x ")
}

KS_results_no_redundant <- KS_results[1:21, ]



### Make pretty table with results CAUSING PROBLEMS WITH KNITTING
...
write.csv2(KS_results, row.names = FALSE, file = "/Users/tess/Documents/work/KS_results_table_final")
...
all_param <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[null_model_conditional,]$ratio), sizw = na.omit(single_disturbence[null_model_conditional,]$sizw))

oregon_model_conditional <- null_model_conditional &  single_disturbence$state =="OR"
ic=c(1.416, 0.984, 0.444, 0.101)
oregon_param <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[ oregon_model_conditional,]$ratio), sizw = na.omit(single_disturbence[oregon_model_conditional,]$sizw))

ic=c(0.0005,0.5,1,1)
florida_model_conditional <- null_model_conditional &  single_disturbence$state =="FL"
florida_param <- optim(ic,MMLnLpower,method="L-BFGS-B",ratio = na.omit(single_disturbence[florida_model_conditional,]$ratio), sizw = na.omit(single_disturbence[florida_model_conditional,]$sizw))
...
ic=c(0.0005,0.5,1,1)
fl_8_3 <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[conditional_fl_8_3 ,]$ratio), sizw = na.omit(single_disturbence[conditional_fl_8_3,]$sizw))

fl_8_5 <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[conditional_fl_8_5 ,]$ratio), sizw = na.omit(single_disturbence[conditional_fl_8_5,]$sizw))

or_6_2 <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[conditional_or_6_2 ,]$ratio), sizw = na.omit(single_disturbence[conditional_or_6_2,]$sizw))

or_7_1 <- optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[conditional_or_7_1 ,]$ratio), sizw = na.omit(single_disturbence[conditional_or_7_1,]$sizw))
   
   ## Florida comparison
    dev.null <- 2*florida_param$value       ## null model
    dev.tmt  <- 2*fl_8_3$value + 2*fl_8_5$value      ## model with elevated vs ambient CO2 treatment
    LR       <- dev.null - dev.tmt        ## Likelihood ratio
    pval     <- 1 - pchisq(LR,4)
    pval
   ...
   ## Oregon comparision 
    ...
    dev.null <- 2*oregon_param$value       ## null model
    dev.tmt  <- 2*or_6_2$value + 2*or_7_1$value      ## model with elevated vs ambient CO2 treatment
    LR       <- dev.null - dev.tmt        ## Likelihood ratio
    pval     <- 1 - pchisq(LR,4)
     pval
    
#upper=c(1000,1, 1000,1000),lower=c(0,0, 0, 0)
     
### All disturbence classes
ic=c(2.5,1,1,1)
run_optim <- function (conditional, disturbences_names){
  conditional <- conditional & single_disturbence$dist_name == disturbences_names
  PR <-  optim(ic,MMLnLpower,method="L-BFGS-B", ratio = na.omit(single_disturbence[conditional,]$ratio), sizw = na.omit(single_disturbence[conditional,]$sizw))
  return(PR)
}
...
dev.null <- 2*fl_8_3$value + 2*fl_8_5$value       ##null model, 8 params
    dev.tmt  <- 2*mle_output$florida_8_3[[1]]$value + 2*mle_output$florida_8_3[[2]]$value + 2*mle_output$florida_8_3[[3]]$value + 2*mle_output$florida_8_3[[4]]$value + 2*mle_output$florida_8_3[[5]]$value + mle_output$florida_8_3[[6]]$value + mle_output$florida_8_3[[7]]$value     ## model with 7 * 4 or 28 parameters, so 
    LR       <- dev.null - dev.tmt        ## Likelihood ratio
    pval     <- 1 - pchisq(LR,20)  ## extreamly large pvalue
    pval    
...
###### Disturbence down
    ...
    ic=c(1, 0.95, 0.7029 ,0.0546)
    ic = c(0,0,0,0)
    ic <- c(100,1,100, 100)
    if (names[i] == "Clearcut"){
    ic=c(0.005, 0.95, 0.7029 ,0.0546)
    }
  ...
  dev.null <-  2*all_param$value    ##null model, 4 params
    ...
    LR       <- dev.null - dev.tmt        ## Likelihood ratio
    pval     <- 1 - pchisq(LR,26)  ## extreamly large pvalue
    pval 
...
##  Disturbence down test Florida
  ...
  dev.null <-  2*mle_output$all_dist[[1]]$value + 2*mle_output$all_dist[[2]]$value + 2*mle_output$all_dist[[3]]$value + 2*mle_output$all_dist[[4]]$value + 2*mle_output$all_dist[[5]]$value + mle_output$all_dist[[6]]$value + mle_output$all_dist[[7]]$value    ##null model, 8 params
    dev.tmt  <- 2*mle_output$all_dist_fl[[1]]$value + 2*mle_output$all_dist_fl[[2]]$value + 2*mle_output$all_dist_fl[[3]]$value + 2*mle_output$all_dist_fl[[4]]$value + 2*mle_output$all_dist_fl[[5]]$value + mle_output$all_dist_fl[[6]]$value + mle_output$all_dist_fl[[7]]$value + 2*mle_output$all_dist_or[[1]]$value + 2*mle_output$all_dist_or[[2]]$value + 2*mle_output$all_dist_or[[3]]$value + 2*mle_output$all_dist_or[[4]]$value + 2*mle_output$all_dist_or[[5]]$value + 2*mle_output$all_dist_or[[6]]$value + 2*mle_output$all_dist_or[[7]]$value   ## model with 14 * 4 or 56 parameters, minus 28 parameters 
    LR       <- dev.null - dev.tmt        ## Likelihood ratio
    pval     <- 1 - pchisq(LR,28)  
    pval 
...
## Ploting curves of edge/interior ratio
```{r, echo = TRUE}
...
### Lumping Disturbences together

### Trying the built in aprouaches 

```{r}


## Testing a glm bioomial with a cauchit link funtion
#lm <- glm(final$ratio ~ final$sizw, family=binomial(link="cauchit"))
#confid <- confint(lm)


#plot(lm$model$`final$sizw`, lm$model$`final$ratio`, xlim = c(0, 1000))
#lines(lm$model$`final$sizw`, lm$fitted.values, type = "p", col = "red", xlim = c(0, #1000))
#lines(sort(lm$model$`final$sizw`),sort(((lm$fitted.values #-lm$coefficients[1])/lm$coefficients[2])*confid[2] + confid[1]), col = "red")
#lines(sort(lm$model$`final$sizw`),sort(((lm$fitted.values-lm$coefficients[1])/lm$coef#ficients[2])*confid[4] + confid[3]), col = "blue")

## Different Disturbences

thining_only_lm  <- glm(thining_only$ratio ~ thining_only$sizw, family=binomial(link="cauchit"))
thining_only_ci <- confint(thining_only_lm)
ggplot()+
  geom_hex( aes(x = thining_only_lm$model$`thining_only$sizw`, y = thining_only_lm$model$`thining_only$ratio`) )+
  geom_line(aes(x = thining_only_lm$model$`thining_only$sizw`, y = thining_only_lm$fitted.values)) +
  geom_line(aes(x = thining_only_lm$model$`thining_only$sizw`, y = (((thining_only_lm$fitted.values- thining_only_lm$coefficients[1])/thining_only_lm$coefficients[2])*thining_only_ci[2] + thining_only_ci[1])), color = "blue") +
  geom_line(aes(x = thining_only_lm$model$`thining_only$sizw`, y = (((thining_only_lm$fitted.values- thining_only_lm$coefficients[1])/thining_only_lm$coefficients[2])*thining_only_ci[4] + thining_only_ci[3])), color = "blue")+
  geom_line(aes( x = thining_only_lm$model$`thining_only$sizw`, y = upper_bound_ratios( thining_only_lm$model$`thining_only$sizw`))) + 
  geom_line(aes( x = thining_only_lm$model$`thining_only$sizw`, y = lower_bound_ratios( thining_only_lm$model$`thining_only$sizw`))) +
  scale_x_continuous(trans="log") 
  geom_line()
