...
color_cheat_sheet$all_eco <- color_cheat_sheet[color_cheat_sheet$dist %in% disturbences_names,]$colors
```
...
options(digits = 3)
dist_size<-list(clearcut_only$sizw, wildland$sizw, wildfire$sizw, prescribed$sizw, thining_only$sizw, other_mechanical$sizw, as.numeric(unknown_only$sizw), herbicide$sizw, mastication$sizw, as.numeric(harvest$sizw), as.numeric(biological$sizw), as.numeric(chemical$sizw), as.numeric(disease$sizw), as.numeric(mixed$sizw) )

names(dist_size) <- c("Clearcut", "Wildland Fire", "Wildfire", "Prescribed", "Thinning","Other mechanical", "Unknown", "Herbicide", "Mastication", "Harvest", "Biological", "Chemical", "Disease", "Mixed" )
...
for (i in 1:(length(names(dist_size))^2)){
  ...
  ## Build factorial function for the calculation of p.val
  fact <- function (num){
    temp = num
    for(i in 1:num){
      temp <- temp + num - i
  ...
  Bonf_val <- 0.05/ (fact(length(all_disturbences)) + 3) # Plus 3 for states and ecoregions
...
hex_conditional <- single_disturbence$sizw > 3 & single_disturbence$dist_name %in%

disturbences_abundence <- single_disturbence[null_model_conditional, ] %>%
...
scale_x_continuous(trans="log") +
scale_fill_gradient(trans = "log", breaks = c(1,10,100, 500),  low = "#bdbdbd", high = "#636363") +
   theme(panel.background = element_blank(),
      #axis.text.y = element_blank(),
      #axis.title.y = element_blank(),
      #axis.ticks.y = element_blank(),
...
lower <- lower_bound_ratios(seq)
upper <- upper_bound_ratios(seq)
#seq <- 1: max(single_disturbence[dist_con,]$sizw)
  
#fake_data_frame <- as.data.frame(cbind(lower, upper))
  
plot[[i]] <- single_disturbence %>%
 ggplot(aes(sizw, ratio), show.legend = TRUE) +
  geom_hex(data = single_disturbence[null_model_conditional & single_disturbence$dist_name == dist_order[i], ], aes(sizw, ratio)) +
  geom_line( aes(x =seq   , y = (upper )), color = "black")+
  geom_line( aes(x =seq  , y = (lower )), color = "black")+
 geom_line( aes(x =seq, (all_dist[[i]][2]*seq^all_dist[[i]][3])/(all_dist[[i]][1] + seq^all_dist[[i]][3]) ), color = color_cheat_sheet[color_cheat_sheet$dist == dist_order[i],]$colors)+
